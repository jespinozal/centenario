...
import getFechasSaldo from "@salesforce/apex/CEN_SimulacionPrepago.getFechasSaldo";
import CEN_N_CUOTAS_DEL_SALDO_DEL_PRECIO__C_FIELD from "@salesforce/schema/Opportunity.CEN_N_Cuotas_del_Saldo_del_precio__c";

// eslint-disable-next-line @lwc/lwc/no-leading-uppercase-api-name
export default class lwcSimuladorPrepagoOportunidad extends LightningElement {
  isModalOpen = false;
  @api recordId;
  @api IdOportunidad;
  ...
  @wire(getRecord, {
    recordId: "$recordId",
    fields: [CEN_N_CUOTAS_DEL_SALDO_DEL_PRECIO__C_FIELD]
  })
  ...
  connectedCallback() {
    // console.log('recordId antes de getFechasSaldo', this.recordId);
    // if (this.recordId) {
    //   getFechasSaldo({ oppId: this.recordId })
    //     .then((resp) => {
    //       this.fechaPrimeraCuotaSaldo = resp.fechaPrimeraCuotaSaldo;
    //       this.fechaUltimaCuotaSaldoOriginal = resp.fechaUltimaCuotaSaldo;
    //     })
    // }
  }
  ...
  oppMapper(data) {
    if (data) {
      ...
      getFechasSaldo({ oppId: this.recordId }).then((resp) => {
        this.fechaPrimeraCuotaSaldo = resp.fechaPrimeraCuotaSaldo;
        this.fechaUltimaCuotaSaldoOriginal = resp.fechaUltimaCuotaSaldo;
        this.cuotaSaldoAnteriorAPrepago = resp.cuotaSaldoAnteriorAPrepago;
        this.fechaPrepago = resp.fechaPrimeraCuotaSaldo;
      });
    }
  }
  ...
  handleInputChange(event) {
    if (event.target.dataset.type === "Monto_Prepago") {
      ...
      if (this.cuotaMantenida === false) {
        console.log("fechaPrepago", this.fechaPrepago);
        console.log("fechaUltimaCuotaSaldo", this.fechaUltimaCuotaSaldo);
        getCuotasRestantes({
          fechaPrepago: this.fechaPrepago,
          fechaUltimaCuotaSaldo: this.fechaUltimaCuotaSaldoOriginal
        }).then((resp) => {
  ...
  consultaPrepago() {
    ...
    if (
      ...
      getSimulacionPrepago({
        ...
        oppId: this.IdOportunidad,
        isSimulation: true,
        dataItemFila: this.filaItem,
        crearProformaItem: this.crearProformaItem
      })
        .then((resp) => {
          // llenando lo campos calculados en el simulador
          this._resp = resp;
          console.log("resp en js");
          console.log(resp);
